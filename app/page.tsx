...
const NotebookContent = ({ userVocab }) => {
  // 1. SÉCURITÉ : On s'assure que la liste existe, sinon on prend une liste vide
  const safeVocab = Array.isArray(userVocab) ? userVocab : [];

  // 2. ANTI-DOUBLONS VOCABULAIRE
  // On utilise une Map pour stocker les éléments uniques par mot espagnol (item.es)
  ...
  // 3. ANTI-DOUBLONS GRAMMAIRE
  // On utilise une Map pour stocker les éléments uniques par titre (item.title)
  const uniqueGrammarMap = new Map();
  safeVocab.filter(c => c.type === 'grammar').forEach(item => {
     if (!uniqueGrammarMap.has(item.title)) {
       uniqueGrammarMap.set(item.title, item);
     }
  });
    ...
    <div className="max-w-4xl mx-auto w-full p-4 md:p-8 pb-24">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="bg-indigo-50 text-indigo-600 px-4 py-2 rounded-lg font-bold text-sm">
          {/* On affiche le total des éléments UNIQUES */}
          {vocabItems.length + grammarItems.length} Éléments
        </div>
      </div>
