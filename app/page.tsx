...
import { 
  Flame, ChevronRight, X, Check, Trophy, User, Book, Zap, Edit3, BookOpen, LogOut, Save, GraduationCap, PlayCircle, Lock, LayoutDashboard, Library, AlertCircle, Mail, Bell, Settings, Loader2, CloudUpload, Volume2, Download, Printer, PenTool, Hammer, ArrowRight, RotateCcw, Table, Map, CheckCircle, Star, BrainCircuit, Target, MessageCircle, Ear
} from 'lucide-react';
  ...
  const handlePrintPDF = async (lessonId) => {
    ...
    element.innerHTML = `
      <style>
        ...
        .pdf-container { font-family: 'Inter', sans-serif; color: #1e293b; background-color: white; padding: 20px; width: 100%; }
        .header { display: flex; justify-content: space-between; align-items: center; border-bottom: 4px solid #facc15; padding-bottom: 20px; margin-bottom: 30px; }
        .logo { font-size: 24px; font-weight: 900; color: #0f172a; } .logo span { color: #dc2626; }
        .title-box { text-align: right; } h1 { font-size: 14px; margin: 0; text-transform: uppercase; letter-spacing: 1px; color: #64748b; } .lesson-id { font-size: 32px; font-weight: 900; color: #4f46e5; line-height: 1; }
        h2 { font-size: 16px; color: #4f46e5; text-transform: uppercase; border-bottom: 2px solid #e2e8f0; padding-bottom: 8px; margin-top: 25px; margin-bottom: 15px; page-break-after: avoid; }
        .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }
        .card { background: #f8fafc; border: 1px solid #e2e8f0; border-left: 4px solid #facc15; border-radius: 8px; padding: 12px; page-break-inside: avoid; }
        .es { font-size: 16px; font-weight: 800; color: #0f172a; margin-bottom: 4px; } .fr { font-size: 14px; color: #64748b; font-weight: 600; } .context { font-size: 11px; color: #94a3b8; font-style: italic; margin-top: 6px; }
        .grammar-box { background: #eef2ff; border: 2px solid #c7d2fe; border-radius: 12px; padding: 15px; margin-bottom: 15px; page-break-inside: avoid; }
        .grammar-title { color: #3730a3; font-weight: 800; margin-bottom: 8px; font-size: 16px; }
        table { width: 100%; border-collapse: collapse; margin-top: 5px; } td { padding: 6px; border-bottom: 1px solid #cbd5e1; font-size: 13px; } td:first-child { color: #64748b; width: 30%; } td:nth-child(2) { font-weight: 800; color: #1e293b; }
        .structure-box { background: #fff; border: 2px dashed #94a3b8; border-radius: 12px; padding: 15px; margin-bottom: 15px; text-align: center; page-break-inside: avoid; }
      ...
      <div class="pdf-container">
        ...
        ${grammar.length > 0 ? `<h2>🧠 Grammaire</h2>${grammar.map(g => `<div class="grammar-box"><div class="grammar-title">${g.title}</div><p style="margin:0; color:#4338ca; font-size:13px;">${g.description}</p><table>${g.conjugation.map(row => `<tr><td>${row.pronoun}</td><td>${row.verb}</td><td style="text-align:right; font-style:italic; color:#94a3b8;">${row.fr}</td></tr>`).join('')}</table></div>`).join('')}` : ''}
        ${structures.length > 0 ? `<h2>🏗️ Structures Clés</h2>${structures.map(s => `<div class="structure-box"><div style="font-weight:800; margin-bottom:5px;">${s.title}</div><span class="formula">${s.formula}</span><p style="margin-top:8px; font-size:13px;">Ex: <strong>${s.example}</strong></p><div style="font-size:11px; color:#64748b; margin-top:4px;">💡 ${s.note}</div></div>`).join('')}` : ''}
...
const SidebarDesktop = ({ userData, currentView, onChangeView, onLogout, onUpload }) => (
  <div className="hidden md:flex flex-col w-72 bg-white border-r border-slate-200 h-full p-6">
    ...
    <nav className="flex-1 space-y-2">
      ...
      <SidebarLink icon={MessageCircle} label="Histoires" active={currentView === 'reading' || currentView === 'story'} onClick={() => onChangeView('reading')} />
      <SidebarLink icon={Trophy} label="Classement" active={currentView === 'leaderboard'} onClick={() => onChangeView('leaderboard')} />
      <SidebarLink icon={BrainCircuit} label="Entraînement" active={currentView === 'tests' || currentView === 'quiz'} onClick={() => onChangeView('tests')} />
...
const MobileBottomNav = ({ currentView, onChangeView }) => (
  <div className="md:hidden bg-white border-t border-slate-100 p-2 pb-6 flex justify-around items-center text-slate-400 z-30">
    ...
    <NavBtn icon={MessageCircle} label="Histoires" active={currentView === 'reading' || currentView === 'story'} onClick={() => onChangeView('reading')} />
    <NavBtn icon={Trophy} label="Ligue" active={currentView === 'leaderboard'} onClick={() => onChangeView('leaderboard')} />
    <NavBtn icon={BrainCircuit} label="Quiz" active={currentView === 'tests' || currentView === 'quiz'} onClick={() => onChangeView('tests')} />
...
const NotebookContent = ({ userVocab }) => {
  ...
  const IRREGULAR_VERBS = new Set(["Ser", "Estar", "Ir", "Tener", "Haber", "Hacer", "Poder", "Querer", "Decir", "Ver", "Dar", "Saber", "Salir", "Poner", "Venir", "Oír", "Traer", "Conocer", "Caber", "Andar"]);
  const vocabItems = sourceList.filter(item => { if (!item || !item.es || item.type !== 'swipe') return false; const contextSafe = item.context ? item.context.toLowerCase() : ""; if (contextSafe.includes('verbe') || DATA_BANK.verbs.some(v => v.es === item.es)) return false; return true; }).filter((item, index, self) => index === self.findIndex((t) => t.es === item.es));
  const grammarItems = sourceList.filter(c => c && c.type === 'grammar').filter(item => { let vName = item.verb; if (!vName) vName = item.title.split('(').pop().replace(')', '').trim(); return IRREGULAR_VERBS.has(vName); }).filter((item, index, self) => index === self.findIndex((t) => t.title === item.title));
  const TENSES_DATA = { "Présent": [{ title: "-AR (Hablar)", endings: ["-o", "-as", "-a", "-amos", "-an"] }, { title: "-ER (Comer)", endings: ["-o", "-es", "-e", "-emos", "-en"] }, { title: "-IR (Vivir)", endings: ["-o", "-es", "-e", "-imos", "-en"] }], "Imparfait": [{ title: "-AR (Hablaba)", endings: ["-aba", "-abas", "-aba", "-ábamos", "-aban"] }, { title: "-ER / -IR (Comía)", endings: ["-ía", "-ías", "-ía", "-íamos", "-ían"] }], "Futur": [{ title: "Tous les verbes", endings: ["Infinitif +", "-é", "-ás", "-á", "-emos", "-án"] }], "Passé Simple": [{ title: "-AR (Hablé)", endings: ["-é", "-aste", "-ó", "-amos", "-aron"] }, { title: "-ER / -IR (Comí)", endings: ["-í", "-iste", "-ió", "-imos", "-ieron"] }] };
  const count = vocabItems.length + grammarItems.length;

  return (
    <div className="max-w-4xl mx-auto w-full p-4 md:p-8 pb-24">
      <div className="flex items-center justify-between mb-8"><h2 className="text-2xl md:text-3xl font-black text-slate-900">Lexique</h2><div className="bg-indigo-50 text-indigo-600 px-4 py-2 rounded-lg font-bold text-sm">{count} Éléments</div></div>
      <div className="mb-8 bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
        <div className="flex justify-between items-center mb-6"><h3 className="font-bold text-slate-800 flex items-center gap-2"><Table size={20} className="text-indigo-600"/> Tableaux de Conjugaison</h3><button onClick={() => setShowReference(!showReference)} className="text-sm text-indigo-600 font-bold hover:underline">{showReference ? "Masquer" : "Afficher"}</button></div>
        {showReference && (<div className="animate-in fade-in slide-in-from-top-2 duration-300"><div className="flex gap-2 overflow-x-auto pb-4 mb-2 no-scrollbar">{Object.keys(TENSES_DATA).map(tense => (<button key={tense} onClick={() => setActiveTense(tense)} className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors ${activeTense === tense ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>{tense}</button>))}</div><div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">{TENSES_DATA[activeTense].map((group, idx) => (<div key={idx} className="bg-slate-50 p-4 rounded-xl border border-slate-200"><h4 className="font-bold text-indigo-600 mb-3 text-center text-sm">{group.title}</h4><div className="flex flex-wrap justify-center gap-2">{group.endings.map((end, i) => (<span key={i} className="bg-white px-2 py-1 rounded text-slate-700 text-xs font-mono border border-slate-100 shadow-sm">{end}</span>))}</div></div>))}</div></div>)}
      </div>
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4"><h3 className="font-bold text-slate-400 uppercase tracking-wider text-sm flex items-center gap-2"><BookOpen size={18} /> Verbes Irréguliers</h3><div className="space-y-4 max-h-[500px] overflow-y-auto pr-1">{grammarItems.length > 0 ? grammarItems.map((item, index) => (<div key={`gram-${index}`} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200"><div className="flex justify-between items-center mb-3"><h4 className="font-bold text-indigo-600">{item.title}</h4><span className="text-[10px] bg-red-100 text-red-600 px-2 py-1 rounded-full font-bold uppercase">Irrégulier</span></div><div className="bg-slate-50 rounded-xl overflow-hidden text-sm border border-slate-100">{item.conjugation && item.conjugation.map((row, idx) => (<div key={idx} className={`flex justify-between items-center p-2.5 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}><span className="text-slate-400 w-16 sm:w-20 shrink-0">{row.pronoun}</span><span className="font-bold text-slate-800 flex-1 text-center">{row.verb}</span><span className="text-slate-400 text-xs w-20 sm:w-auto text-right italic shrink-0">{row.fr}</span></div>))}</div></div>)) : (<div className="p-6 text-center text-slate-400 border-2 border-dashed rounded-xl text-sm">Aucun verbe irrégulier appris pour le moment.</div>)}</div></div>
      </div>
    </div>
  );
};
...
const ProfileContent = ({ userData, email, onLogout, onUpload }) => (
  <div className="max-w-2xl mx-auto w-full p-6 md:p-12 space-y-8">
    ...
    <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 space-y-6">
      <div className="flex items-center gap-4"><div className="w-16 h-16 bg-indigo-100 rounded-full flex items-center justify-center text-2xl font-bold text-indigo-600">{userData?.name?.charAt(0).toUpperCase()}</div><div><p className="font-bold text-slate-900 text-lg">{userData?.name}</p><p className="text-slate-400 text-sm">{email}</p></div></div>
      <div className="grid grid-cols-3 gap-4 text-center py-4 border-y border-slate-100"><div><p className="text-2xl font-black text-slate-900">{userData?.xp}</p><p className="text-xs text-slate-400 uppercase font-bold">XP Total</p></div><div><p className="text-2xl font-black text-slate-900">{userData?.streak}</p><p className="text-xs text-slate-400 uppercase font-bold">Série</p></div><div><p className="text-2xl font-black text-slate-900">{userData?.level}</p><p className="text-xs text-slate-400 uppercase font-bold">Niveau</p></div></div>
      <button onClick={onUpload} className="w-full bg-indigo-50 text-indigo-600 py-4 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-indigo-100 transition-colors border border-indigo-100"><CloudUpload size={20} /> Réinitialiser le Contenu</button>
...
const LessonEngine = ({ content, onComplete, onExit, isExam }) => { 
  ...
  const handleNext = () => { if (currentIndex + 1 >= content.length) { setProgress(100); setTimeout(() => onComplete(150, score), 500); } else { setProgress(((currentIndex + 1) / content.length) * 100); setCurrentIndex(prev => prev + 1); } }; 
  const handleScore = (correct) => { if (correct) setScore(s => s + 1); };
  const handlePrev = () => { if (currentIndex > 0) { setCurrentIndex(prev => prev - 1); setProgress(((currentIndex - 1) / content.length) * 100); } }; 
  useEffect(() => { if (currentIndex === 0 && currentCard?.es) speak(currentCard.es); }, [currentIndex]); 
  return (
    <div className="h-full w-full flex flex-col bg-slate-50">
      <div className="px-6 py-4 md:py-6 flex items-center gap-6 bg-white border-b border-slate-100 z-10"><button onClick={onExit} className="text-slate-400 hover:text-slate-600 transition-colors"><X size={24} /></button><div className="flex-1 h-4 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-teal-500 transition-all duration-500 ease-out rounded-full" style={{ width: `${progress}%` }}></div></div>{isExam && <div className="font-black text-indigo-600">{score} / 20</div>}</div>
      <div className="flex-1 flex items-center justify-center p-4 overflow-hidden"><div className="w-full max-w-md aspect-[3/4] md:aspect-auto md:h-[600px] perspective-1000">
...
const SwipeCard = ({ data, onNext, onPrev }) => {
  ...
  const handleSwipe = (dir) => { setSwiped(dir); setTimeout(() => { if (dir === 'left') onPrev(); else onNext(); }, 250); };
  useEffect(() => { const handleKeyDown = (e) => { if (e.key === "ArrowLeft") handleSwipe('left'); if (e.key === "ArrowRight") handleSwipe('right'); if (e.key === " " || e.key === "Enter") { e.preventDefault(); speak(data.es); } }; window.addEventListener("keydown", handleKeyDown); return () => window.removeEventListener("keydown", handleKeyDown); }, [data]);
  return (
    <div className={`w-full h-full bg-white rounded-3xl shadow-xl border-b-8 border-slate-100 flex flex-col relative transition-all duration-300 ${swiped === 'left' ? '-translate-x-[150%] rotate-[-20deg] opacity-0' : ''} ${swiped === 'right' ? 'translate-x-[150%] rotate-[20deg] opacity-0' : ''}`}>
      <div className="absolute top-4 right-4 z-10"><button onClick={(e) => { e.stopPropagation(); speak(data.es); }} className="p-3 bg-slate-100 rounded-full hover:bg-slate-200 text-indigo-600 shadow-sm active:scale-95 transition-all"><Volume2 size={24} /></button></div>
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center space-y-6"><h2 className="text-5xl md:text-6xl font-black text-slate-800">{data.es}</h2><div className="w-16 h-1 bg-slate-100 rounded-full"></div><div className="bg-indigo-50 px-6 py-3 rounded-2xl border border-indigo-100"><p className="text-xl md:text-2xl font-bold text-indigo-600">{data.en}</p></div><p className="text-sm text-slate-400 italic">"{data.context}"</p></div>
      <div className="p-6 pb-8 flex justify-center gap-8"><button onClick={() => handleSwipe('left')} className="w-16 h-16 rounded-full bg-red-50 border-2 border-red-100 text-red-500 flex items-center justify-center hover:bg-red-100 active:scale-95 transition-all"><RotateCcw size={32} strokeWidth={3} /></button><button onClick={() => handleSwipe('right')} className="w-16 h-16 rounded-full bg-teal-500 border-b-4 border-teal-700 text-white flex items-center justify-center hover:bg-teal-400 hover:scale-105 active:scale-95 active:border-b-0 active:translate-y-1 transition-all"><Check size={32} strokeWidth={3} /></button></div>
    </div>
  );
};
const InputCard = ({ data, onNext, isExam, onScore }) => { 
  ...
  const addChar = (char) => { if (isSubmitted) return; setVal(prev => prev + char); inputRef.current?.focus(); };
  const checkAnswer = (e) => { 
    ...
    if (isExam) { setIsSubmitted(true); if (onScore) onScore(isCorrect); speak(isCorrect ? "¡Muy bien!" : "Incorrecto"); setTimeout(() => { onNext(); }, 2500); } else { if (isCorrect) { speak("¡Muy bien!"); setTimeout(onNext, 1500); } else { speak("Inténtalo de nuevo"); } }
  }; 
  return (
...
const GrammarCard = ({ data, onNext }) => {
  ...
  return (
    <div className="w-full h-full bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-right duration-500">
      <div className="bg-indigo-600 p-8 md:p-10 text-white text-center relative"><button onClick={(e) => { e.stopPropagation(); speak(data.title); }} className="absolute top-4 right-4 p-2 bg-white/20 rounded-full hover:bg-white/30 text-white"><Volume2 size={20} /></button><h3 className="text-3xl md:text-4xl font-black">{data.title}</h3><p className="text-indigo-200 mt-2">{data.description}</p></div>
      <div className="flex-1 p-6 md:p-10 flex flex-col justify-between bg-slate-50">{revealed ? (<div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">{data.conjugation.map((row, idx) => (<div key={idx} className="flex justify-between items-center p-4 border-b border-slate-100 last:border-0"><span className="text-slate-400 font-medium w-1/3">{row.pronoun}</span><span className="text-indigo-600 font-black text-xl w-1/3 text-center">{row.verb}</span><span className="text-slate-300 text-sm w-1/3 text-right italic">{row.fr}</span></div>))}</div>) : (<div className="flex-1 flex items-center justify-center"><p className="text-slate-400 italic">Clique ci-dessous pour révéler la conjugaison</p></div>)}<button onClick={revealed ? onNext : () => setRevealed(true)} className="w-full mt-6 bg-yellow-400 text-slate-900 py-5 rounded-2xl font-bold text-xl shadow-lg hover:bg-yellow-300 active:scale-95 transition-all">{revealed ? 'J\'ai compris' : 'Voir la Conjugaison'}</button></div>
    </div>
  );
};
...
const DailyReadingContent = ({ userLevel }) => { 
  ...
  return (
    <div className="max-w-2xl mx-auto w-full p-6 pb-24 space-y-8"><div className="text-center"><span className="bg-indigo-100 text-indigo-600 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">Lecture du jour</span><h2 className="text-3xl font-black text-slate-900 mt-2">{reading.title_es}</h2><div className="flex justify-center gap-2 mt-2"><span className="text-slate-400 italic text-sm">{reading.difficulty}</span><span className="bg-green-100 text-green-700 text-xs px-2 py-0.5 rounded font-bold">Adapté à ton niveau</span></div></div><div className="bg-white p-8 rounded-3xl shadow-lg border-b-4 border-slate-100 relative overflow-hidden group"><div className="absolute top-0 right-0 bg-yellow-400 text-yellow-900 text-xs font-bold px-3 py-1 rounded-bl-xl">Español</div><button onClick={() => speak(reading.text_es)} className="absolute top-6 right-6 p-3 bg-indigo-50 text-indigo-600 rounded-full hover:bg-indigo-100 transition-colors" title="Écouter le texte"><Volume2 size={20} /></button><p className="text-xl text-slate-800 leading-relaxed font-medium mt-4">{reading.text_es}</p></div><div className="flex justify-center"><button onClick={() => setShowTranslation(!showTranslation)} className="flex items-center gap-2 text-slate-500 hover:text-indigo-600 font-bold transition-colors">{showTranslation ? <div className="flex items-center gap-2"><X size={16}/> Masquer la traduction</div> : <div className="flex items-center gap-2"><Check size={16}/> Voir la traduction</div>}</button></div>{showTranslation && (<div className="bg-slate-50 p-8 rounded-3xl border-2 border-dashed border-slate-200 animate-in fade-in slide-in-from-top-4 duration-500"><h3 className="text-slate-400 font-bold text-sm uppercase mb-2 tracking-wider">Français</h3><h4 className="text-lg font-bold text-slate-700 mb-2">{reading.title_fr}</h4><p className="text-slate-600 leading-relaxed">{reading.text_fr}</p></div>)}<div className="bg-blue-50 p-4 rounded-xl flex gap-3 items-start"><div className="bg-blue-100 p-2 rounded-lg text-blue-600 shrink-0"><BrainCircuit size={20} /></div><div><p className="font-bold text-blue-800 text-sm">Conseil du coach</p><p className="text-blue-600 text-xs mt-1">Lis le texte à haute voix 3 fois. La première fois pour déchiffrer, la deuxième pour comprendre, la troisième pour l'intonation.</p></div></div></div>
  );
};